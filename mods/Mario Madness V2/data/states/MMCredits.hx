...
function create(){
    ...
    var bg2:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('menus/credits/credits1'));
    bg2.updateHitbox();
    bg2.screenCenter();
    ...
    for(i in 1...7){
        var shit:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/credits/scroll/scroll'+ i ));
        shit.screenCenter();
        shit.alpha = 0;
    ...
    for(i in 0...41){
        ...
        var icon:FlxSprite = new FlxSprite(creditX - 480, creditY - 160).loadGraphic(Paths.image('menus/credits/Char' + Std.string(i + 1)));
    ...
    specialThanksBox.screenCenter(FlxAxes.X);
    ...
    specialThanksTitle.screenCenter(FlxAxes.X);
    ...
    for(i in 0...infoArray.length){
        ...
        specialThanksText.screenCenter(FlxAxes.X);
    }


    var theend:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('menus/credits/theend'));
    theend.updateHitbox();
    theend.screenCenter();
    ...
    if(autoscroll){
        ...
        camFollow.setPosition(640, -800);
        FlxG.sound.playMusic(Paths.music('creditsFinale'), 1);

        FlxTween.tween(camFollow, {y: 13800}, 188, {
            onComplete: function(twn:FlxTween)
            {
                FlxG.sound.music.volume = 0;
                FlxG.camera.fade(FlxColor.BLACK, 2, false, function()
                {
                    camFollow.setPosition(640, 0);
                    lyricsBox.visible = false;
                    lyricsText.visible = false;
                    theend.visible = true;
...
function update(elapsed:Float){
    if(!autoscroll){
        ...
        if(camFollow.y != newPos){
            var lerpVal:Float = FlxMath.bound(elapsed * 5, 0, 1);
            camFollow.y = FlxMath.lerp(camFollow.y, newPos, lerpVal);
        }
